...
class ParticleSwarmCETSP(CETSPSolver):
    ...
    def _apply_swap_sequence(self, tour: List[int], swaps: List[Tuple[int, int]], 
...
class DiscretePSOCETSP(CETSPSolver):
    ...
    def _initialize_swarm(self) -> None:
        ...
        self.positions = np.random.uniform(0, self.n_customers, 
                                          (self.n_particles, self.n_customers))
        ...
        # Evaluate all
        self.fitnesses = np.array([self._evaluate_position(pos) 
    ...
    def _create_greedy_tour(self) -> List[int]:
        ...
        while unvisited:
            best_idx = min(unvisited, key=lambda idx: 
                          np.linalg.norm(
