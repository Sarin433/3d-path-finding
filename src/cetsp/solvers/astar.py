...
class CETSPState:
    ...
    def __eq__(self, other: object) -> bool:
        if not isinstance(other, CETSPState):
            return False
        return (
            np.allclose(self.position, other.position, atol=0.01) and
...
class AStarCETSP(CETSPSolver):
    ...
    def _greedy_complete(self, current_pos: np.ndarray, visited: frozenset[int], 
                         path_positions: list, path_covered: list) -> tuple:
