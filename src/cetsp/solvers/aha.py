...
class ArtificialHummingbirdCETSP(CETSPSolver):
    ...
    def _select_food_source(self, hb_idx: int) -> int:
        ...
        if visits.sum() > 0:
            ...
            probs = nectar_weights / nectar_weights.sum() if nectar_weights.sum() > 0 else np.ones(self.n_hummingbirds) / self.n_hummingbirds
            probs[hb_idx] = 0
    ...
    def solve(self) -> CETSPSolution:
        ...
        for iteration in range(self.n_iterations):
            ...
            if iteration > 0 and iteration % 30 == 0:
                ...
                sorted_hbs = sorted(enumerate(self.hummingbirds), 
...
class EnhancedAHACETSP(ArtificialHummingbirdCETSP):
    ...
    def _adapt_parameters(self, iteration: int, total_iterations: int, 
            ...
            for hb_idx in range(self.n_hummingbirds):
                ...
                if r < self.guided_prob:
                    ...
                    if random.random() < 0.3:
                        new_tour = self._opposition_based_tour(
                            self.hummingbirds[hb_idx].tour
                        )
                    else:
                        new_tour = self._migration_foraging(hb_idx)
            ...
            if iteration > 0 and iteration % 25 == 0:
                ...
                for i in range(replace_count):
                    ...
                    if random.random() < 0.5:
                        ...
                        opp_tour = self._opposition_based_tour(
                            self.personal_best[idx].tour
                        )
