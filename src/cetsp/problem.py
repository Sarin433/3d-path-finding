...
class CETSPPath:
    ...
    def add_waypoint(self, point: np.ndarray, covered: Optional[CETSPNode] = None):
...
class CETSP:
    ...
    def compute_optimal_waypoint(
        ...
        target: CETSPNode,
        next_target: Optional[CETSPNode] = None
    ) -> np.ndarray:
        """
        ...
        if target.radius > 0:
            ...
            if exit_dist > 0:
                ...
                # Compare paths
                path1_dist = np.linalg.norm(entry - from_point) + \
                            np.linalg.norm(next_entry - entry)
                path2_dist = np.linalg.norm(entry - from_point) + \
    ...
    def solve(self, method: str = "greedy", **kwargs) -> CETSPSolution:
        ...
        """
        from .solvers import (
            GreedyCETSP, AStarCETSP, GeneticCETSP, AdaptiveGeneticCETSP,
            AntColonyCETSP, MaxMinAntSystem, AntColonySystemCETSP,
            ArtificialHummingbirdCETSP, EnhancedAHACETSP,
            ParticleSwarmCETSP, AdaptivePSOCETSP, DiscretePSOCETSP
        )
    ...
    def visualize(self, solution: Optional[CETSPSolution] = None):
        """
