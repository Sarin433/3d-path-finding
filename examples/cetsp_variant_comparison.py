...
def run_variant(
    cetsp: CETSP, method: str, family: str, variant_name: str, params: dict, seed: int
) -> VariantResult:
...
def run_all_variants(cetsp: CETSP, n_customers: int, seed: int = 42) -> list[VariantResult]:
    ...
    result = run_variant(
        cetsp, "adaptive_genetic", "Genetic Algorithm", "Adaptive GA", ga_params, seed
    )
    print(f"       Distance: {result.distance:.2f}, Time: {result.time_ms:.2f} ms")
    results.append(result)
...
def print_summary(results: list[VariantResult]) -> None:
    ...
    for rank, r in enumerate(sorted_results, 1):
        ...
        print(
            f"{rank:<5} {r.family:<18} {r.variant:<15} {r.distance:>10.2f} {r.time_ms:>12.2f} {gap:>7.1f}%"
        )
...
def plot_matplotlib(results: list[VariantResult], save_path: str = None) -> plt.Figure:
    ...
    # Add value labels
    for bar, dist in zip(bars, distances, strict=False):
        gap = (dist - best_distance) / best_distance * 100
        label = f"{dist:.1f} ({gap:+.1f}%)" if gap > 0 else f"{dist:.1f} (best)"
        ax1.text(
            bar.get_width() + 5, bar.get_y() + bar.get_height() / 2, label, va="center", fontsize=9
        )
    ...
    for family, color in family_colors.items():
        ...
        if family_results:
            ...
            for t, d, lbl in zip(times, dists, labels, strict=False):
                ax2.annotate(
                    lbl.replace(" ", "\n"),
                    (t, d),
    ...
    if len(pareto_points) > 1:
        ...
        ax2.plot(
            pareto_times, pareto_dists, "k--", alpha=0.5, linewidth=1.5, label="Pareto frontier"
        )
...
def plot_plotly(results: list[VariantResult], save_path: str = None) -> go.Figure:
    ...
    for family, color in family_colors.items():
        ...
        if family_results:
            ...
            hover_text = [
                f"{v}<br>Distance: {d:.2f}<br>Gap: {g:.1f}%<br>Time: {r.time_ms:.1f}ms"
                for v, d, g, r in zip(variants, distances, gaps, family_results, strict=False)
            ]
    ...
    for family, color in family_colors.items():
        ...
        if family_results:
            ...
            hover_text = [
                f"<b>{v}</b><br>Distance: {d:.2f}<br>Gap: {g:.1f}%<br>Time: {t:.1f}ms"
                for v, d, g, t in zip(variants, distances, gaps, times, strict=False)
            ]
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    mode="markers+text",
                    name=family,
                    marker={"size": 15, "color": color, "line": {"width": 1, "color": "black"}},
                    text=variants,
    ...
    fig.update_layout(
        title={"text": "CETSP Algorithm Variant Comparison", "font": {"size": 16}},
        height=500,
        width=1200,
        showlegend=True,
...
def plot_family_comparison(results: list[VariantResult], save_path: str = None) -> go.Figure:
    ...
    for family, (row, col) in family_positions.items():
        ...
        if family_results:
            variants = [r.variant for r in family_results]
            distances = [r.distance for r in family_results]
            [r.time_ms for r in family_results]
            ...
            fig.add_trace(
                go.Bar(
                    ...
                    y=distances,
                    marker_color=color,
                    text=[f"{d:.1f}<br>({g:.1f}%)" for d, g in zip(distances, gaps, strict=False)],
                    textposition="outside",
                    showlegend=False,
    ...
    fig.update_layout(
        title={"text": "Variant Comparison by Algorithm Family", "font": {"size": 16}},
        height=600,
        width=1000,
    )
...
def create_summary_table(results: list[VariantResult], save_path: str = None) -> go.Figure:
    ...
    fig.update_layout(
        title={"text": "Algorithm Variant Rankings", "font": {"size": 16}},
...
def plot_individual_tours_matplotlib(
    cetsp: CETSP, results: list[VariantResult], save_path: str = None
) -> plt.Figure:
    ...
    plt.suptitle(
        "Tour Visualization for Each Variant (sorted by distance)",
        fontsize=14,
        fontweight="bold",
        y=1.02,
    )
...
def plot_individual_tours_plotly(
    cetsp: CETSP, results: list[VariantResult], save_path: str = None
) -> go.Figure:
    ...
    # Update layout
    fig.update_layout(
        title={"text": "Tour Visualization for Each Variant", "font": {"size": 18}},
        height=400 * n_rows,
        width=350 * n_cols,
        showlegend=False,
    )
    ...
    for i in range(n_results):
        ...
        fig.update_layout(
            **{
                scene_name: {
                    "xaxis": {"range": [0, 100], "title": "X"},
                    "yaxis": {"range": [0, 100], "title": "Y"},
                    "zaxis": {"range": [0, 50], "title": "Z"},
                    "aspectmode": "manual",
...
def plot_top_tours_comparison(
    cetsp: CETSP, results: list[VariantResult], top_n: int = 4, save_path: str = None
) -> go.Figure:
    ...
    for idx, r in enumerate(sorted_results):
        ...
        gap_str = "BEST" if gap == 0 else f"+{gap:.1f}%"
        titles.append(
            f"#{idx + 1} {r.variant} ({r.family})<br>Distance: {r.distance:.1f} ({gap_str})"
        )
    ...
    fig.update_layout(
        title={"text": f"Top {top_n} Tour Comparison", "font": {"size": 18}},
        height=600,
        width=400 * top_n,
...
def main():
    ...
    plot_individual_tours_matplotlib(cetsp, results, save_path=output_dir / "variant_all_tours.png")
    ...
    print("\n[Plotly - All Tours]")
    plot_individual_tours_plotly(cetsp, results, save_path=output_dir / "variant_all_tours.html")

    print("\n[Plotly - Top 4 Tours]")
    fig_tours = plot_top_tours_comparison(
        cetsp, results, top_n=4, save_path=output_dir / "variant_top_tours.html"
    )
